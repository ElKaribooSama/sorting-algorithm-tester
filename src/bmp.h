...
struct BMP {
    ...
    void get_region(uint32_t x0, uint32_t y0, uint32_t x1, uint32_t y1, uint8_t *out) {
        if(x0 > x1 || y0 > y1) return;
        uint32_t channels = bmp_info_header.bit_count / 8;

        for (uint32_t y = 0; y < (y1 - y0); ++y) {
            for (uint32_t x = 0; x < (x1 - x0); ++x) {
                uint32_t dpos = channels * ((bmp_info_header.height - 1 - (y0 + y)) * bmp_info_header.width + (x0 + x));
                out[channels * (y * (x1 - x0) + x) + 0] = data[dpos + 0];
                out[channels * (y * (x1 - x0) + x) + 1] = data[dpos + 1];
                out[channels * (y * (x1 - x0) + x) + 2] = data[dpos + 2];
                if (channels == 4) {
                    out[channels * (y* (x1 - x0) + x) + 3] = data[dpos + 3];
                }
            }
        }
    }
